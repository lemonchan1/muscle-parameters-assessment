...
CONFIG_NAME = "custom/fifth_uctransnet.yaml"
...
img_train = glob(os.path.join('inputs', "train", 'images', '*' + config['dataset']['img_ext']))
img_train = [os.path.splitext(os.path.basename(p))[0] for p in img_train]
img_val = glob(os.path.join('inputs', "val", 'images', '*' + config['dataset']['img_ext']))
img_val = [os.path.splitext(os.path.basename(p))[0] for p in img_val]
img_test = glob(os.path.join('inputs', "test", 'images', '*' + config['dataset']['img_ext']))
img_test = [os.path.splitext(os.path.basename(p))[0] for p in img_test]
train_img_ids = img_train
val_img_ids = img_val
test_img_ids = img_test
# if config['transforms'] == None:
train_transform = None
val_transform = None
tr_dataset = Dataset(
    img_ids=train_img_ids,
    img_dir=os.path.join('inputs', "train", 'images'),
    mask_dir=os.path.join('inputs', "train", 'masks'),
...
print(f"Torch device: {device}, GPU: {config['run']['gpu']}")

# ## Metrics

# In[ ]:
num_classes = config['dataset']['num_classes']  # 包括背景类（值为0）和8个区域（值为1到8）

metrics = torchmetrics.MetricCollection(
    [
        torchmetrics.F1Score(num_classes=num_classes, average='macro', task='multiclass',ignore_index=0),
        torchmetrics.Accuracy(task='multiclass', num_classes=num_classes,ignore_index=0),
        torchmetrics.Dice(num_classes=num_classes,ignore_index=0),
        # HausdorffDistance(num_classes=num_classes),
        torchmetrics.Precision(task='multiclass', num_classes=num_classes,ignore_index=0),
        torchmetrics.Recall(task='multiclass', num_classes=num_classes,ignore_index=0),
        torchmetrics.Specificity(task='multiclass', num_classes=num_classes,ignore_index=0),
        torchmetrics.JaccardIndex(num_classes=num_classes, task='multiclass',ignore_index=0)
    ],
...
def test(model, te_dataloader):
    save_outputs_dir = f"{config['model']['save_dir']}/outputs"
    if not os.path.isdir(save_outputs_dir):
    ...
    with torch.no_grad():
        ...
        for batch_data in tqdm(te_dataloader):
            ...
            for i in range(len(tpm)):
                fid = tid[i]
                tmp = Image.fromarray(tpm[i].astype(np.uint8), mode='L')
                tmp.save(os.path.join(save_outputs_dir, fid + '.png'))
        evaluator = make_serializeable_metrics(evaluator.compute())
...
# te_metrics.compute()
# 保存测试指标
with open(f"{config['model']['save_dir']}/test_metrics.txt", "w") as f:
    for key, value in te_metrics.items():
        f.write(f"{key}: {value}\n")
...
# plt.show()
plt.savefig(f"{config['model']['save_dir']}/result.png")
