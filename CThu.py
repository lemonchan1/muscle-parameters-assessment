...
def load_slice_ranges():
    ...
    try:
        ...
        print(f"警告：配置文件 {config_path} 不存在，将使用所有可用切片")
        return {}
    except Exception as e:
        print(f"读取配置文件失败: {str(e)}，将使用所有可用切片")
        return {}
...
def calculate_ct_density():
    ...
    for label_file in tqdm(label_files, desc="Processing files"):
        ...
        use_all_slices = file_config is None
        ...
        total_slices = label_data.shape[2]  # 获取总切片数

        # 确定切片范围
        if use_all_slices:
            # 使用所有切片
            start, end = 0, total_slices - 1
            print(f"信息: {label_file} 使用所有 {total_slices} 个切片")
        else:
            # 使用配置的切片范围
            ...
            if start > end:
                start, end = end, start
            print(f"信息: {label_file} 使用配置的切片范围 {start}-{end} (共 {total_slices} 个切片)")
        ...
        for label in unique_labels:
            ...
            summary_data.append({
                ...
                'Start_Slice': start,
                'End_Slice': end,
                'Total_Slices_Processed': total_slices  # 添加总切片数
            })
